    ...
    <div class="container py-8 mx-auto relative">
      ...
      <div class="max-w-6xl mx-auto relative">
        ...
        <div class="mt-16 mb-12 text-center">
          <h1 class="text-4xl md:text-5xl font-bold mb-3 text-primary bg-clip-text">{{ t('software.title') }}</h1>
        ...
        <div class="space-y-16">
          <section 
            ...
            class="scroll-mt-20"
          >
            <h2 class="text-2xl font-semibold mb-3 mt-8 pb-2 border-b border-gray-200">{{ t(`software.${category.id}`) }}</h2>
...
const scrollToSection = (sectionId: string) => {
  ...
  if (element) {
    const elementPosition = element.getBoundingClientRect().top + window.scrollY - 140
    window.scrollTo({
      top: elementPosition,
      behavior: 'smooth'
    })
...
const handleScroll = () => {
  const sectionElements: SectionElement[] = categories.map((category: Category) => ({
    id: category.id,
